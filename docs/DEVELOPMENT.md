...
## Backup System Implementation (January 7, 2026)

### Overview
A comprehensive backup system was implemented to protect against character encoding corruption that causes Japanese and Nepali characters to display as "?????". This system provides automated recovery capabilities for AI agents and manual recovery options for developers.

### Backup System Location
- **Backup folder:** `backup/` at project root
- **Backup files:** 12 UTF-8 BOM encoded text files (one per test category)
- **Total questions backed up:** 394+ questions across all tests
- **Encoding:** UTF-8 with BOM (mandatory for Devanagari and CJK scripts)

### Backup Files Structure
Each backup file (`{test-name}-backup.txt`) contains:
```
=== TEST NAME ===
Duration: {minutes}
...
--- QUESTION 1 ---
Text: [Japanese/Nepali/English question text]
A) [Option A text]
B) [Option B text]  
C) [Option C text]
D) [Option D text]
Correct Answer: [A|B|C|D]
Image Path: [path if applicable]
Audio Path: [path if applicable]
...
### Backup Files List
1. **jft-basic-1-backup.txt** → `tests/jft-basic-1/jft-basic-1-data.js` (55 questions)
2. **jft-basic-2-backup.txt** → `tests/jft-basic-2/jft-basic-2-data.js` (55 questions)
3. **jft-basic-3-backup.txt** → `tests/jft-basic-3/jft-basic-3-data.js` (55 questions)
4. **jft-basic-4-backup.txt** → `tests/jft-basic-4/jft-basic-4-data.js` (60 questions)
5. **nursing-care-japanese-1-backup.txt** → `tests/nursing-care-japanese-1/nursing-care-japanese-1-data.js` (17 questions)
6. **nursing-care-japanese-2-backup.txt** → `tests/nursing-care-japanese-2/nursing-care-japanese-2-data.js` (17 questions)
7. **nursing-care-nepali-1-backup.txt** → `tests/nursing-care-nepali-1/nursing-care-nepali-1-data.js` (50 questions)
8. **nursing-care-nepali-2-backup.txt** → `tests/nursing-care-nepali-2/nursing-care-nepali-2-data.js` (50 questions)
9. **agriculture-backup.txt** → `tests/agriculture/agriculture-data.js` (30 questions)
10. **food-service-backup.txt** → `tests/food-service/food-service-data.js` (30 questions)
11. **building-cleaning-backup.txt** → `tests/building-cleaning/building-cleaning-data.js` (35 questions)
12. **ground-handling-backup.txt** → `tests/ground-handling/ground-handling-data.js` (40 questions)

### Backup Documentation Files
Located in `backup/` folder:

1. **00_START_HERE.txt** - Quick reference and entry point for all backup operations
2. **BACKUP_README.md** - Comprehensive master documentation with complete format specifications
3. **RESTORATION_GUIDE.txt** - Technical procedures for manual and automated restoration (AI-ready specifications)
4. **BACKUP_INVENTORY.txt** - Complete inventory with test mappings and question counts
5. **CREATION_SUMMARY.txt** - Implementation summary with timestamp and procedures

### Restoration Procedures

#### When Tests Get Corrupted
If any test displays questions as "?????" (character encoding corruption):

**Quick Steps:**
1. Check `backup/{test-name}-backup.txt` for intact question data
2. Reference `backup/RESTORATION_GUIDE.txt` for detailed restoration procedures
3. Update the corresponding `tests/{test-name}/{test-name}-data.js` file using backup data
4. Verify encoding is UTF-8 with BOM when saving

**For AI Agents:**
- Read `backup/RESTORATION_GUIDE.txt` for complete automation specifications
- Use backup text files as reference source (plain text, UTF-8 with BOM)
- Regenerate JavaScript data files following PowerShell import-questions.ps1 pattern
- Verify character rendering in browser after restoration

#### Character Encoding Requirements
- **Backup files:** UTF-8 with BOM encoding (critical for Nepali Devanagari and Japanese CJK)
- **JavaScript files:** Must also be UTF-8 with BOM when containing Japanese/Nepali
- **Text editors:** Use VS Code or similar with UTF-8 BOM support for editing

#### Important Notes
- All 394+ questions preserved with original character sets intact
- Backup files are read-only references (do not edit them)
- Original test data structure documented in RESTORATION_GUIDE.txt
- PowerShell script `scripts/import-questions.ps1` can regenerate data files from backups
- Audio and image paths are preserved in backup metadata
